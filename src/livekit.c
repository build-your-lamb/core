  ...
  switch (response->message_case) {
    ...
    case LIVEKIT__SIGNAL_RESPONSE__MESSAGE_TRICKLE:
      ...
      if (response->trickle->target == LIVEKIT__SIGNAL_TARGET__SUBSCRIBER) {
        livekit_webrtc_subscriber_add_ice_candidate(response->trickle->candidateinit);
      } else if (response->trickle->target == LIVEKIT__SIGNAL_TARGET__PUBLISHER) {
        livekit_webrtc_publisher_add_ice_candidate(response->trickle->candidateinit);
      }
      break;
    case LIVEKIT__SIGNAL_RESPONSE__MESSAGE_UPDATE:
...
static int livekit_callback(struct lws* wsi, enum lws_callback_reasons reason, void* user, void* in, size_t len) {
  switch (reason) {
    ...
    case LWS_CALLBACK_CLIENT_RECEIVE: {
      ...
      if (lws_is_final_fragment(wsi)) {
        Livekit__SignalResponse* response = livekit__signal_response__unpack(NULL, received_offset, received_buffer);
...
//  nng_dial(video_recv_sock, "inproc://video.compressed", NULL, NNG_FLAG_NONBLOCK);
