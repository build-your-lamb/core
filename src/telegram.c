...
void send_message(CURL *curl, long long chat_id, const char *text)
{
    ...
    snprintf(post, sizeof(post),
        "chat_id=%lld&text=%s", chat_id, escaped);
...
LOGI("received: %s", buf.data);
	  ...
	  int64_t chat_id_value = chat_id->valuedouble;
	    ...
	    break;
          ...
          cJSON_Delete(root);
	  send_message(curl, chat_id_value, "Message received!");
        } while (0);
...
void app_telegram_quit(void) {
    ...
    // on nng_listen or nng_recvmsg (though nng_listen might not block in this pub/sub setup,
    // future blocking calls might). For now, it's global and closed in main.
    // If app_telegram_main is in a separate thread, this would signal it to exit.
}
