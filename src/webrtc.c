...
static void onopen(void* user_data) {
  ...
  // peer_connection_send_data(g_subscriber_peer_connection, "Hello from onopen", strlen("Hello from onopen"));
}
...
int livekit_webrtc_subscriber_is_connected() {
  if (g_subscriber_peer_connection) {
    return peer_connection_get_state(g_subscriber_peer_connection) == PEER_CONNECTION_CONNECTED;
  }
...
void livekit_webrtc_set_remote_description(const char* sdp, const char* type) {
  if (strcmp(type, "offer") == 0) {
    ...
    peer_connection_set_remote_description(g_subscriber_peer_connection, sdp, SDP_TYPE_OFFER);
  } else if (strcmp(type, "answer") == 0) {
    LOGD("Setting remote SDP for publisher peer: %s", sdp);
    peer_connection_set_remote_description(g_publisher_peer_connection, sdp, SDP_TYPE_ANSWER);
  }
}

static void livekit_webrtc_add_ice_candidate(const char* candidate, PeerConnection* pc) {
...
static void livekit_webrtc_publisher_onstatechange(PeerConnectionState state, void* userdata) {
  LOGI("Publisher peer connection state changed: %s", peer_connection_state_to_string(state));
}

static void livekit_webrtc_publisher_onicecandidate(char* description, void* userdata) {
}

static void livekit_webrtc_subscriber_onstatechange(PeerConnectionState state, void* userdata) {
  LOGI("Subscriber peer connection state changed: %s", peer_connection_state_to_string(state));
}

static void livekit_webrtc_subscriber_onicecandidate(char* description, void* userdata) {
}
...
void livekit_webrtc_create_peer_connections() {
  ...
  peer_connection_oniceconnectionstatechange(g_subscriber_peer_connection,
                                             livekit_webrtc_subscriber_onstatechange);

  peer_connection_ondatachannel(g_subscriber_peer_connection, onmessage, onopen, onclose);
  ...
  peer_connection_oniceconnectionstatechange(g_publisher_peer_connection,
                                             livekit_webrtc_publisher_onstatechange);

  peer_connection_ondatachannel(g_publisher_peer_connection, onmessage, onopen, onclose);
  pthread_create(&g_subscriber_thread, NULL, (void*)livekit_webrtc_subscriber_thread, NULL);

  pthread_create(&g_publisher_thread, NULL, (void*)livekit_webrtc_publisher_thread, NULL);
  pthread_create(&g_data_handler_thread, NULL, (void*)livekit_webrtc_data_handler_thread, NULL);
}
